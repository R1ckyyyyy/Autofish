...
- 📊 **强大的数据统计** - 内置历史与今日统计面板，提供总数、传说数量、脱钩率等关键指标，并附带品质分布饼图。
- 📖 **详细的钓鱼记录** - 自动记录每条鱼的名称、重量、品质和时间，支持按品质筛选和搜索。
- 🔤 **OCR 文本识别** - 使用 RapidOCR 识别鱼的名称、品质和重量
- 🎨 **现代化界面** - 基于 PySide6 和 QFluentWidgets 的现代GUI，简洁美观。
- 悬浮窗** - 可选的迷你悬浮窗，实时显示状态。
- 实时日志** - 在主界面显示详细的运行日志，方便追踪程序状态。
- 调试截图** - 使用 F10 键可快速生成用于调试的截图。
- 桶满提醒** - 游戏内鱼桶满时，程序能自动识别并暂停。
- ⌨️ **全局热键** - 使用F2键启动/暂停
